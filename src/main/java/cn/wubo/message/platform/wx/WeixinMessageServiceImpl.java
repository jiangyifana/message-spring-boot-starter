...
public class WeixinMessageServiceImpl extends AbstractSendService<WeixinPrpperties.Message> {
    ...
    public String sendMarkdown(WeixinPrpperties.Message aliasProperties, MarkdownContent content) {
        JSONObject result = WeixinUtils.getToken(Objects.requireNonNull(aliasProperties.getCorpid()),
                Objects.requireNonNull(aliasProperties.getCorpsecret()));
        JSONObject jo = request(aliasProperties,
                content.getContentParams(aliasProperties.getAlias()).getWeixinMessage());
        jo.put("msgtype", "markdown");
        JSONObject jsonObject = new JSONObject();
        jsonObject.put("content", ContentUtils.toMarkdown(content, "\n"));
        jo.put("markdown", jsonObject);
    ...
    private JSONObject request(WeixinPrpperties.Message aliasProperties, Map<String, Object> params) {
        ...
        if (params.containsKey(TOUSER)) {
            jo.put(TOUSER, params.get(TOUSER));
        }
        if (params.containsKey(TOPARTY)) {
            jo.put(TOPARTY, params.get(TOPARTY));
        }
        if (params.containsKey(TOTAG)) {
            jo.put(TOTAG, params.get(TOTAG));
        }
